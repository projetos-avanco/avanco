...
function retornaLogsDeAcoesDiarias($db, $carteira)
{
  ...
  if ($resultado = $db->query($query)) {
    ...
    # recuperando logs existentes no período atual
    while ($registro = $resultado->fetch_assoc()) {

      $logs[] = array('id_chat' => $registro['id_chat']);
 ...
 */
 function removeAcoesDiariasRepetidas($db, $acoes, $logs)
 {
   $contador = count($acoes);

   for ($i = 0; $i < $contador; $i++) {

     for ($j = 0; $j < count($logs); $j++) {

       if ($acoes[$i]['id_chat'] == $logs[$j]['id_chat']) {
