...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  # verificando se o id do endereço foi enviado
  if ((!empty($_POST['endereco']['id-endereco'])) && is_numeric($_POST['endereco']['id-endereco'])) {
    $endereco['id_endereco'] = $_POST['endereco']['id-endereco'];
  } else {
    $validacao['flag']    = true;
    $validacao['erros'][] = 'Código do Endereço não foi enviado. Informe ao Wellington Felix.';
  }

  # verificando se o id do cnpj da empresa foi enviado
  if ((!empty($_POST['endereco']['id-cnpj'])) && is_numeric($_POST['endereco']['id-cnpj'])) {
    $endereco['id_cnpj'] = $_POST['endereco']['id-cnpj'];
  } else {
    $validacao['flag']    = true;
  ...
  if ($validacao['flag']) {
    ...
    # redirecionando usuário para página de edição do endereço
    ...
    recebeNovoEnderecoParaEdicao($endereco);
  }
}
