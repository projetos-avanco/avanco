$(function() {
  ...
  $(document).on('click', '#btn-consultar', function(e) {
    ...
    if (gestao.id === '') {
      ...
      $.ajax({
        type: 'post',
        url: '../../../database/functions/schedule/customers/ajax/dados_paginacao_gestao_prox.php',
        dataType: 'json',
        data: {
          id: gestao.id,
        ...
        success: function(dados) {
          ...
          table += '<table class="table table-condensed" id="relatorio-gestao-prox">' +
            '<thead>'                                      +
              '<tr>'                                       +
          ...
          for (var i = 0; i < dados.length; i++) {
            ...
            tbody += 
              '<td>' +
                '<button class="btn btn-success btn-sm btn-block" id="visualizar-atendimento-gestao-prox" type="button" value="' + dados[i].id + '">' +
          ...
          $('#tabela-relatorio-gestao-prox').html(table);
  
          // paginando a tabela
          $('#relatorio-gestao-prox').DataTable({
            "aaSorting": [[3, "desc"]],   
            "oLanguage": {
