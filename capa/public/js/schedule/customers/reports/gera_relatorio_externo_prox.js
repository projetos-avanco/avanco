$(function() {
  ...
  $(document).on('click', '#btn-consultar', function(e) {
    ...
    if (externo.id === '') {
      ...
      $.ajax({
        type: 'post',
        url: '../../../database/functions/schedule/customers/ajax/dados_paginacao_externo_prox.php',
        dataType: 'json',
        data: {
          id: externo.id,
        ...
        success: function(dados) {
          ...
          table += '<table class="table table-condensed" id="relatorio-externo-prox">' +
            '<thead>'                                      +
              '<tr>'                                       +
          ...
          for (var i = 0; i < dados.length; i++) {
            ...
            tbody += 
              '<td>' +
                '<button class="btn btn-success btn-sm btn-block" id="visualizar-atendimento-prox" type="button" value="' + dados[i].id + '">' +
            ...
            if (dados[i].relatorio_entregue === 'Sim') {
              tbody += 
                '<td>' +
                  '<button class="btn btn-success btn-sm btn-block" id="visualizar-relatorio-prox" type="button" value="' + dados[i].issue + '">' +
                    '<i class="fa fa-eye" aria-hidden="true"></i> Visualizar' +
                  '</button' +
                '</td>';
            ...
            if (dados[i].pesquisa_realizada === 'Sim') {
              tbody += 
                '<td>' +
                  '<button class="btn btn-success btn-sm btn-block" id="visualizar-pesquisa-prox" type="button" value="' + dados[i].id_pesquisa + '">' +
                    '<i class="fa fa-eye" aria-hidden="true"></i> Visualizar' +
                  '</button' +
                '</td>';
          ...
          $('#tabela-relatorio-externo-prox').html(table);
  
          // paginando a tabela
          $('#relatorio-externo-prox').DataTable({
            "aaSorting": [[3, "desc"]],   
            "oLanguage": {
