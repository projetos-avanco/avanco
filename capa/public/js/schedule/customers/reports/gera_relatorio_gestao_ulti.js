$(function() {
  ...
  $(document).on('click', '#btn-consultar', function(e) {
    ...
    if (gestao.id === '') {
      ...
      $.ajax({
        type: 'post',
        url: '../../../database/functions/schedule/customers/ajax/dados_paginacao_gestao_ulti.php',
        dataType: 'json',
        data: {
          id: gestao.id,
        ...
        success: function(dados) {
          ...
          table += '<table class="table table-condensed" id="relatorio-gestao-ulti">' +
            '<thead>'                                      +
              '<tr>'                                       +
          ...
          for (var i = 0; i < dados.length; i++) {
            ...
            tbody += 
              '<td>' +
                '<button class="btn btn-success btn-sm btn-block" id="visualizar-atendimento-gestao-ulti" type="button" value="' + dados[i].id + '">' +
          ...
          $('#tabela-relatorio-gestao-ulti').html(table);
  
          // paginando a tabela
          $('#relatorio-gestao-ulti').DataTable({
            "aaSorting": [[3, "desc"]],   
            "oLanguage": {
