    ...
    if (status === 'Confirmado') {
      swal({
        ...
        if (confirmar) {
          ...
          url = tmp[0]+'//'+tmp[2]+'/'+tmp[3]+'/'+tmp[4]+'/public/views/schedule/cancela_atendimento_remoto.php?id=' + id + '&id-cnpj=' + idCnpj + '&id-contato=' + idContato;
      ...
      swal({
        ...
        if (confirmar) {
          $.ajax({
            type: 'get',
            url: '../../../app/requests/get/schedule/remote/recebe_atendimento.php?id=' + id,
            dataType: 'json',            
            success: function(retorno) {
              swal({
                icon: 'success',
                title: 'Atendimento Remoto',
                text: 'Atendimento Remoto cancelado com sucesso.',
                buttons: {                  
                  cancel: {
                    text: 'Fechar',
                    closeModal: true,
